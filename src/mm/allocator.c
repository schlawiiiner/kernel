...
void deallocate_pool(Allocator* allocator, int page_table_free_index, int stack_index) {
    ...
    memset((uint64_t)allocator->stack_ptr_table_base[stack_index]-allocator->stack_size*sizeof(uint64_t), 0x0, allocator->stack_size*sizeof(uint64_t));
    // NOTE: the stack pointer must point here to the top of the stack, otherwise we would free a pool that is beeing used. This fact we use
    // to clear the stack (stack base = stack pointer - stack size), but we never check if the stack is full, so be carefull if you call deallocate_pool
...
uint64_t malloc(int size) {
    ...
    if ((uint64_t)allocator->stack_ptr_to_hint[0] & (allocator->stack_size*8 - 1)) {
